        ...
        actions: hasSelectedCubes
            ? [
                TextButton.icon(
                  ...
                  label: Text('Enviar a Recepción ${provider.selectedCubeIds.length} cubos'),
                ),
              ]
            : null,
      ),
  ...
  Future<void> _showSendToReceptionDialog(BuildContext context) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '¿Está seguro que desea enviar $cubeCount ${cubeCount == 1 ? 'cubo' : 'cubos'} a recepción?\n\n'
    ...
    try {
      ...
      for (final cubeId in transportProvider.selectedCubeIds) {
        final details = await transportProvider.fetchCubeDetailsRaw(cubeId, suppressAuthHandling: true);
        guideCodes.addAll(details.guides.map((g) => g.packageCode).whereType<String>());
      }

      developer.log('Collected ${guideCodes.length} guide codes from selected cubes', name: 'WarehouseTransitScreen');
      ...
      if (guideCodes.isNotEmpty) {
        final guidesUpdate = await context.read<GuideProvider>().updateGuideStatus(
          UpdateGuideStatusRequest(guides: guideCodes.toList(), newStatus: 'TransitToWarehouse'),
        );

        if (!guidesUpdate.isSuccessful) {
          if (!context.mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(guidesUpdate.messageDetail ?? guidesUpdate.message ?? 'Error actualizando guías a Tránsito'),
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(guidesUpdate.messageDetail ?? guidesUpdate.message ?? 'Guías actualizadas a Tránsito'),
      ...
      final response = await transportProvider.changeSelectedCubesState(TransportCubeState.SENT);
      developer.log('Cube state changed to Sent - Success: ${response.isSuccessful}', name: 'WarehouseTransitScreen');
      ...
      if (response.isSuccessful) {
        ...
        if (!context.mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(response.messageDetail ?? response.message ?? '✅ $cubeCount ${cubeCount == 1 ? 'cubo enviado' : 'cubos enviados'} a Tránsito'),
          backgroundColor: Colors.green,
          duration: const Duration(seconds: 4),
        ));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(response.messageDetail ?? response.message ?? 'Error al cambiar estado de cubos a Sent'),
          backgroundColor: Colors.red,
        ));
      }
    } catch (e) {
